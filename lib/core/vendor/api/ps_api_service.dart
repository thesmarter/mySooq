...
class PsApiService extends PsApi {
  ...
  /// Get vendor user list
  ///
  Future<PsResource<List<VendorUser>>> getVendorSearchList(String loginUserId,Map<dynamic, dynamic> jsonMap,int? limit,int? offset) async {
    final String url =
        '${PsUrl.ps_search_vendor_url}login_user_id=$loginUserId&limit=$limit&offset=$offset';
    return await postData<VendorUser, List<VendorUser>>(VendorUser(), url,jsonMap);
  }
...
// {{ server_url }}vendor/paypal/token?vendor_id=1&login_user_id=1
  Future<PsResource<VendorPaypalToken>> getVendorPaypalToken(
    ...
    print('api=>$loginUserId');
    print('api=>$vendorId');
    print('api=>$headerToken');
    final String url =
